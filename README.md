...
1. **PostgreSQL + pgvector** — Postgres must have the `vector` extension (`CREATE EXTENSION vector;` on your database). Without `POSTGRES_USER` in `.env`, Django falls back to SQLite, which does **not** support RAG embeddings.
